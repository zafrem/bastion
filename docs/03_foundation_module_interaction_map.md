...
```python
# Python (Navigator is a Python service in v3)
class HookManager:
...
```python
# Python (Anchor is a Python service in v3)
# Same HookManager pattern as Navigator

# Hook points:
# anchor.embedding.secured
# anchor.response.analyzed
# anchor.response.verified
```
...
### 9.1 Standard Topology (v3 Polyglot)
...
LB → Sentinel(Go) → Vault(Go) → Navigator(Py) → Anchor(Py) → LLM
(JSON-over-gRPC between modules; wire format identical for Go+Python)

Event path (asynchronous):
All modules → NATS → Tracker
(JSON payload; Go and Python use same event structure)

Storage:
Vault → PostgreSQL (tokens)
Navigator → Qdrant (vectors, Python qdrant-client)
...
| 2.0 | 2026-05-26 | Polyglot architecture: Navigator/Anchor → Python; deployment topology updated; hook code examples updated for Python modules; wire contract confirmed unchanged |
